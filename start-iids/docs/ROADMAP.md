...
| 4 — EEA engines | TEI → EFA → EcoFA → SFA → EEA aggregation | **DONE** (formulas + engines + tests); coefficients are placeholders in tests only, never marked APPROVED (P0-02 open) |
...
12. TSI_norm computable with coherent baseline — DONE
...
30. Golden regression tests approved — **PARTIAL**: CQS (sec. 19.5) and the P-TSA AHP-formula self-consistency check pass against published numbers; the P-TSA z-score targets are blocked on a real fixture (see ADR-011)
...
1. Obtain real coefficient values from plant/process engineering and load them as
   a `DRAFT` → reviewed → `APPROVED` `dim_coefficient_set` (never skip the review).
2. Get IT to supply real MES/SCADA/ERP/HR/LIMS field names and complete
   `audit_source_mapping` + per-source YAML contracts (P0-03).
3. Resolve the five ADR-011 open items with the actual SRC-TEI/EFA/EcoFA/SFA/RP74
   manual text and an owner sign-off.
4. Build the Power BI semantic model against `mv_intelligent_industry_state` /
   the read-only API (sec. 38).
5. Run the Stage 9 checklist (spec sec. 65) against a staging environment with
